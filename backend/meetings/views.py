...
# from .utils import check_conflict, generate_ics
# import json
...
#         # Check for conflicts before adding the participant
#         meeting = Meeting.objects.get(id=meeting_id)
#         if check_conflict(email, meeting.start_time, meeting.end_time):
#             return JsonResponse({'error': 'Conflict detected for this participant'}, status=400)
...
#         response = JsonResponse({'ics_file': ics_content})
...
def add_participant(request):
    if request.method == "POST":
        ...
        # Check if the participant is already registered for this specific meeting
